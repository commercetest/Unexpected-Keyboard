...
public class KeyboardAccessibilityActionsInstrumentedTest {
    ...
    public void modifierKeys_reportStateDescription() {
        ...
        final KeyboardData keyboard = buildModifierKeyboard();
        final Keyboard2View[] holder = new Keyboard2View[1];
    ...
    @Test
    public void frenchLocale_returnsFrenchLabels() {
        ...
        Context frContext = base.createConfigurationContext(cfg);
        AccessibilityHelper frHelper = new AccessibilityHelper(frContext);

        String atDesc = frHelper.getKeyDescription(KeyValue.makeCharKey('@'));
        assertEquals("arobase", atDesc);

        String modState = frHelper.getModifierStateDescription(true, true);
        assertEquals("verrouillé", modState);
    }
